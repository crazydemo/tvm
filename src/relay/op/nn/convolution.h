...
bool Conv2DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  if (!trans_kernel_layout.defined()) {
    reporter->GetDiagCtx().Emit(
        ...
        << kOIHW <<"." << " The provided layout is: " << kernel_layout);
    return false;
  }
  ...
  if (param->kernel_size.defined() && param->channels.defined()) {
    ...
    if (is_group) {
      ...
      wshape = {{param->groups, indexdiv(param->channels, param->groups), indexdiv(dshape_nchw[1], param->groups), param->kernel_size[0],
                 param->kernel_size[1]}};
...
bool Conv2DTransposeRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  ICHECK(trans_kernel_layout.defined())
      ...
      << kIOHW <<"." << " But got " << kernel_layout << " " << kIOHW;
  ...
  if (param->kernel_size.defined() && param->channels.defined()) {
    ...
    if (is_group) {
      // infer weight's shape for depthwise convolution
      wshape = {{param->groups, indexdiv(dshape_nchw[1], param->groups), indexdiv(param->channels, param->groups), param->kernel_size[0],
                 param->kernel_size[1]}};
    } else {
      wshape = {{dshape_nchw[1], indexdiv(param->channels, param->groups),
