...
class DNNLJSONRuntime : public JSONRuntimeBase {
 ...
 private:
  ...
  void Convolution(const size_t& nid) {
    ...
    if (wgh_tr.dims()[2] != src_tr.dims()[1] / groups || wgh_tr.dims()[1] != dst_tr.dims()[1] / groups) {
      auto wgh_croped_dims = wgh_tr.dims();
      wgh_croped_dims[1] = dst_tr.dims()[1]; // wgh_OC = dst_OC
      wgh_croped_dims[2] = src_tr.dims()[1]; // wgh_IC = src_IC
